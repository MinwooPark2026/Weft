...
>
> 영상 한 편의 이미지 50~90장이 **같은 그림체·팔레트·무드**로 보이게 하는 설정. 기본값은 3Blue1Brown식 도식(코드의 `DEFAULT_STYLE`)이지만, **누구나 자기만의 느낌으로 바꿀 수 있다.**
...
> 코드가 `STYLE.txt`를 읽어 모든 생성 프롬프트에 붙인다(`load_style`). 파일이 없으면 기본(`DEFAULT_STYLE`, 3b1b)을 쓴다. → **코드 수정 없이 파일 하나로 영상 룩 전체를 바꾼다.**
