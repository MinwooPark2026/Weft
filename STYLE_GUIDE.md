...
> 영상 한 편의 이미지 50~90장이 **같은 그림체·팔레트·무드**로 보이게 하는 설정. 지금 기본값은 3Blue1Brown식 도식이지만, **누구나 자기만의 느낌으로 바꿀 수 있다.**
...
- **후보 N장 + picker.** `N=2~3 ./run.sh images` 후 picker(`./run.sh pick`)에서 가장 톤이 맞는 걸 고른다. 톤이 튀는 컷은 picker에서 프롬프트 수정 후 `G`로 재생성.
